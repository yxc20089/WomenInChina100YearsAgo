...
| Prepare NER gold work | Candidate workflow implemented; human gold absent | Active OCR can be sampled into a content-addressed administrative packet, blinded reviewer view and strict two-review/adjudication template. The expanded live packet has 150 units across three pages/volumes but remains ineligible: one decade, no issue IDs and fewer than 500 units. |
...
| GraphRAG comparison | Adapters and three-document export implemented; experiment pending | The shared export contains 2,471 cited text regions and 27 accounted empty regions across three pages. Article segmentation, recorded LLM configuration and historian retrieval judgments are required before indexing/model comparison. |
