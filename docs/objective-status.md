...
| Select NER/extraction models | Architecture and executable benchmark boundary implemented | Rules + pinned GLiNER adapters now emit a non-ingestible, multi-OCR-run benchmark artifact with exact split/input/gold mappings. The supervised design pins W2NER code and adds training-issue-only MacBERT domain adaptation to the MacBERT/GujiRoBERTa/SIKU tournament; Otter is rights-blocked, while NuExtract/Qwen remain routed experiment arms. Paired issue-cluster comparison is executable. Current zero-shot outputs disagree severely and are candidates only; no benchmark results or winner exist. |
