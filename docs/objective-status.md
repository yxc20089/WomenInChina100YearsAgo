...
| Select NER/extraction models | Architecture and benchmark contract implemented | Rules + supervised SIKU-BERT span model + GLiNER-X recall union, with NuExtract3 routed challenger. Current zero-shot outputs disagree severely and are candidates only. A double-reviewed historical gold set is the gate. |
...
| LLM research assistant | One-turn safety contract implemented | An OpenAI-compatible adapter generates evidence-bounded briefs when configured. No live model is configured; a persistent multi-turn research conversation remains to implement and evaluate. |
...
retrieval set. The next product bottleneck is a citation-validating multi-turn
assistant over that evidence plane.
