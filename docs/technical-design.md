...
- BGE-M3 at commit `5617a9f61b028005a4858fdac845db406aefb181` produced 1,138 normalized 1,024-dimensional embeddings for the screening smoke run. The active source-resolution pages now have 962 (1924), 1,099 (1925), and 437 (1926) embeddings; all histories remain in PostgreSQL/pgvector and the current OpenSearch alias contains exactly those 2,498 active regions.
- OpenSearch CJK lexical, BGE-M3 dense, and client-side RRF hybrid retrieval return exact S3 volume/page/region polygons and propagate page-quality warnings. Live year-filtered checks ranked exact `否認廣州今戒嚴` first for 1924, `士女` first for 1925 hybrid, and `我國現在通行的西裝大` first for 1926 hybrid; each hit carries the registered derivative and image hash.
- Entity-link persistence retains an explicit NIL option. With no reviewed authority catalog yet, all 115 smoke mentions correctly remain NIL rather than manufacturing people. Relation rules v2 require exact reviewed linked mention spans, compatible argument types, an intervening lexical cue, a bounded gap, no clause boundary and complete source/derivative/image/region provenance. The live pass correctly emits zero claims. In an isolated temporary database, one synthetic school-attendance sentence produced one minimal exact citation, passed claim ingestion, and was then dropped with the entire fixture database; no historical row was changed.
- The local historian queue exposes all 492 persisted NER candidates across the legacy screening, lossless pilot and bounded three-year runs with exact derivative, OCR, input hash/dataset split, model and link-candidate provenance. Dataset/run filters isolate exact cohorts, including 130 page-202 and 86 page-230 GLiNER-X candidates from first-50-region compatibility subsets. Mention acceptance and entity resolution are separate transactional decisions. An isolated live-database test accepted a synthetic span, created a reviewed entity from its NIL candidate, proved idempotent retries, and removed every fixture row; no historical candidate was changed.
...
- The reviewed-only Neo4j projection was live-tested and correctly produced an empty graph from that state. A local FastAPI/researcher UI exposes lexical, dense and hybrid search, page images, historian review, reviewed-only analytical signals, and a scenario-context contract that abstains when no reviewed claims support a reconstruction. Its research chat is multi-turn but stateless: each question retrieves again and accepts at most 12 prior turns as untrusted continuity context. The OpenAI-compatible handoff now requires an immutable model revision; remote use requires HTTPS and explicit data-egress consent; redirects cannot carry context/tokens elsewhere. Missing or foreign citations reject and withhold the raw model text while retaining prompt/context/output/configuration hashes. Provider-reported token usage and contract prices are retained without imputation. A scene additionally requires ordered epistemic sections and a directly reviewed citation inside Direct evidence. The executable generation benchmark freezes prompt inputs and expected support, refuses ineligible historian coverage by default, exports model-blind packets for two independent reviews plus adjudication, and compares adjudicated case scores with deterministic paired bootstrap intervals. It has no live scores or selected model.
...
- The executable `wic-worker` now runs or resumes each page stage. A second page-308 DAG automatically adopted the exact source render, OCR run, embeddings and bounded NER artifact after revalidating their model/configuration/input identities. A fresh render-only job for page 309 decoded the single JBIG2 raster to a 6186×8962 PNG with no geometric transform, recorded source/image/pixel hashes, and correctly labeled the derivative `unreviewed_ingestion` rather than gold. Production fleet controls remain pending.
- Two additional four-stage DAGs completed with zero blocked/dead-letter jobs for volume 202/page 338 and volume 230/page 367. Both used size-verified local source objects, fresh source-resolution renders, sequential bounded-memory OCR, full BGE-M3 embeddings and explicitly first-50-region GLiNER-X candidates. Page 202 produced 962 OCR regions/embeddings and 130 candidates; page 230 produced 437 regions/embeddings and 86 candidates. Their derivatives are `unreviewed_input`, not gold.
...
- `wic-worker --loop` provides bounded polling for stage-specific worker pools. Operators must set positive job and consecutive-idle limits; polling backoff is capped at 60 seconds. Summaries separate adopted from freshly generated artifacts and retain retry/failure counts. Live checks proved both idle-bound exit and one-job `max_jobs` exit; this is a local worker primitive, not a fleet autoscaler.
- Dead-letter replay is explicit and audited. Replaying the forced render failure reset only that job plus its dependency-failure-cancelled OCR descendant, changed the failed batch back to active with one ready/one blocked job, and appended `reopened` events with operator/reason. The worker then rendered page 310 successfully from the verified cache; the remaining OCR was explicitly cancelled to keep the recovery test bounded. Batch-cancelled jobs are not eligible for automatic replay.

The slice intentionally contains no reviewed historical entities or claims. Its reviewed-insight endpoint therefore returns zero items with an explicit warning. A separate exploratory endpoint reports active-OCR theme matches and NER disagreement as evidence-linked machine observations: the current three-page scope contains 2,498 regions, 14,366 OCR characters and 305 candidates across five NER runs. It surfaces one `女學` pattern on the 1924 page and the earlier 1925 `士女`/performance leads, all with scan citations and an explicit warning that absence/frequency is meaningless at this scope. These diagnostics prioritize review and are not historical findings. The system must not be described as a reconstructed knowledge graph until historian review data exists.
...
The target is late-Qing/Republican-era printed Traditional Chinese, not
ancient/Classical Chinese. Assume largely modern/transitional newspaper grammar
and discourse. Adaptation should focus on original character forms, period
names/titles, typography, punctuation, layout and OCR corruption. Ancient-text
pretraining is a secondary mismatch control, not an automatic advantage. The
evidence string is never converted to Simplified Chinese; normalized aliases
may be stored only as separate retrieval/linking features.
...
| MacBERT, revision `a986e00…` + W2NER | Modern/transitional-Chinese supervised NER | **First supervised arm**; 58.26 F1 versus SIKU 56.83 on retyped 1872–1947 *Shen Bao*, but not OCR |
| MacBERT-DAPT + W2NER | Proposed target-domain supervised NER | **Principal domain challenger**; train on training issues only and freeze corpus/checkpoint hashes; no checkpoint or result exists yet |
| mmBERT-base, revision `c595503…` + W2NER | New massively multilingual supervised NER | **Same-head challenger, not new primary**; MIT weights and Chinese coverage, but its paper says it ties XLM-R on NER and identifies tokenizer weakness; no historical/OCR result |
| Chinese ModernBERT-large-WWM, revision `b00f1ff…` + W2NER | Chinese-specific long-context supervised NER | **Tokenizer-gated same-head challenger**; no NER/Traditional/historical/OCR result, and default custom preprocessing changes evidence text, so execution requires an isolated audit, preprocessing disabled and expanded offset qualification |
| GujiRoBERTa-jian-fan, revision `8e75570…` + W2NER | Ancient-text encoder | **License-gated secondary domain-mismatch control**; the project is not ancient Chinese, and its cited head comparison warns of augmented-fold leakage |
| SIKU-BERT, revision `fc656de…` + W2NER | Ancient-book-pretrained encoder with target-period evaluation | **Retain as a control only because the closest retyped newspaper comparison reports it directly**, slightly below MacBERT |
| Otter CE mmBERT, revision `aed019f…` | Compact multilingual open-type NER | **Research-only and rights-blocked**; no checkpoint-weight license and no Chinese-specific, historical, OCR or nested result |
| GLiNER-X large, revision `4a4437f…` | 865M multilingual open-type span NER | **Existing open-span recall challenger**; reported `zh_pud` F1 0.6794, but no Traditional/historical/OCR result |
...
The official [GLiNER-X collection](https://huggingface.co/collections/knowledgator/gliner-x) provides small `d51a098…`, base `a6c7a8f…` and large `4a4437f…` checkpoints, with published generic `zh_pud` F1 of 0.5792, 0.6152 and 0.6794 respectively. Use large as the accuracy-first candidate, base only as an efficiency control if large qualifies or hardware requires it, and small as diagnostic-only; none of the scores establishes Traditional-Chinese newspaper/OCR quality. The first technical smoke test used the explicit multilingual v2.1 checkpoint because its official card identifies it as a 209M multilingual Apache-2.0 model. Its poor unreviewed output on noisy *Shen Bao* OCR confirms that generic model-card claims and confidence scores are not sufficient. High scores such as single-character kinship terms classified as people still occurred. Benchmark future models on corrected gold text and raw OCR separately to distinguish OCR propagation from NER failure.
...
### 7.3 Relation and event extraction decision

No external benchmark settles relation extraction for noisy Republican-era
Chinese newspapers. Use `reviewed-co-mention-rules-v2` as the mandatory
high-precision/abstention baseline. The first supervised hypothesis is a
MacBERT pair classifier using the pinned [DeepKE framework](https://github.com/zjunlp/DeepKE)
and adjudicated argument spans. [PRGC](https://aclanthology.org/2021.acl-long.486/)
is the overlapping-triple challenger, not an assumed improvement. Route
[NuExtract3](https://huggingface.co/numind/NuExtract3) only to difficult or
image-context cases and require verbatim arguments plus exact evidence recovery.
[OneKE](https://github.com/zjunlp/OneKE) is a noncommercial research control.
KnowCoder-X, TRUE-UIE, HistRED, and the retyped *Shen Bao* relation layer remain
research context until executable rights, ontology fit, and target evaluation
are established. Exact revisions and rights are frozen in
`experiments/relation/candidates.json`; there is no production winner.

The executable `wic-relation-benchmark` contract depends on the byte-exact NER
gold file and approved coherent-unit revisions. It derives eligibility rather
than accepting a caller-provided flag, enforces issue-isolated splits, two
independent relation reviews plus separate adjudication, exact corrected/raw
argument and evidence offsets, negative units, complete scan provenance and
immutable model/code revisions. The rule adapter, scorer, and paired
issue-cluster bootstrap comparator are implemented and fixture-tested. An
eligible historical set still requires at least 300 units from 30 issues and
two decades, including the documented positive, negative, per-predicate, and
per-split minima. Model output remains a candidate: claim review and a later
Neo4j rebuild are separate acts.
...
7. Create reviewed article segmentation and historian-authored retrieval questions.
...
| NER | Frozen-backbone W2NER/GlobalPointer/CRF comparison; target-relevant MacBERT/MacBERT-DAPT/mmBERT/qualified Chinese ModernBERT encoder tournament; SIKU control; gated GLiNER-X recall union; locally served Qwen3.5-0.8B and NuExtract3 challengers | Multi-run benchmark harness, issue-cluster comparator and provenance-safe W2NER/noise exporter implemented; pinned mmBERT offset qualifier passed, Chinese ModernBERT is custom-tokenizer blocked, ancient-text encoders are deprioritized, Qwen's backend-neutral exact-offset adapter is pending, and eligible training/scoring remains pending; no winner |
...
| Model handoff | OpenAI-compatible adapter with egress consent, immutable model identity, citation enforcement and untrusted-OCR/history boundary | Local HTTP provider integration and redirect-exfiltration tests pass; hashes, provider usage/cost and scan links render in the UI; approved live model not configured; scenes abstain without directly reviewed claims |
| Evaluation | Frozen OCR/NER/relation/RAG/generation judgments with evidence-validity and task metrics | OCR/NER/relation contracts and scorers, retrieval smoke harness, and grounded-generation objective/blind/paired-human harness implemented; historian-authored sets and live model comparisons pending |
