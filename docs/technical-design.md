...
| Otter CE mmBERT, revision `aed019f…` | Compact multilingual open-type NER | **License-gated open-type challenger**; promising 2026 universal-NER results, but no target historical/OCR evidence |
...
The exact registry, roles and revisions are committed in `experiments/ner/candidates.json`; model names alone are not reproducible selections. Generic leaderboards do not settle this choice. The closest [historical Chinese newspaper study](https://aclanthology.org/2024.lrec-main.35/) used entirely retyped *Shen Bao*, reporting MacBERT 58.26 and SIKU 56.83 F1; it is target-period evidence but explicitly not OCR evidence. A controlled [EvaHan comparison](https://aclanthology.org/2025.alp-1.24/) reports W2NER 88.48 average F1 versus GlobalPointer 87.33 and CRF 86.33 with the same BERT encoder, making W2NER the primary overlap-capable head hypothesis. Otter's [2026 preprint](https://arxiv.org/abs/2601.06347) makes it a compact open-type challenger, not a winner without Chinese historical evaluation. Recent [HisDoc-OCR research](https://aclanthology.org/2026.findings-acl.301/) documents fabricated characters, repetition and semantic drift when general multimodal models read Chinese historical scans. Therefore NuExtract3/Qwen image arms remain separately scored ablations with exact visual/text grounding, never assumed OCR-robust NER.
...
| NER | Rules + MacBERT/GujiRoBERTa/SIKU W2NER tournament; gated Otter/GLiNER-X recall union; NuExtract3 difficult-case challenger | Registry, adjudicated-gold contract and scorer implemented; historian annotation/model comparison pending |
