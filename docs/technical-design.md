...
| GujiRoBERTa-jian-fan, revision `8e75570…` + W2NER | Historical/mixed-script supervised NER | **License-gated supervised arm**; W2NER has the strongest controlled head result, but on clean ancient text |
| SIKU-BERT, revision `fc656de…` + W2NER | Historical-Chinese supervised NER | **Supervised tournament arm**, demoted from assumed primary because the closest newspaper comparison favors MacBERT slightly |
...
| NER | Rules + MacBERT/MacBERT-DAPT/mmBERT/tokenizer-qualified Chinese ModernBERT/GujiRoBERTa/SIKU W2NER tournament; gated GLiNER-X recall union; NuExtract3 difficult-case challenger | Multi-run benchmark harness and issue-cluster comparator implemented; pinned mmBERT offset qualifier passed, Chinese ModernBERT is custom-tokenizer blocked, and all supervised/noise-aware training remains pending; PP-UIE assistance/other rights-blocked arms excluded from automatic union; no winner |
