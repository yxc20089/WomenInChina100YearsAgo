...
The gold set should report strict and partial span F1 by entity type, candidate recall@k, linking accuracy including NIL, relation F1 conditional on correct evidence, calibration and performance by OCR CER/decade. Evaluate paired corrected-text, raw-OCR and observed-confusion augmentation arms; split by issue/date; add hallucinated-span rate, invalid evidence/offset rate, throughput and peak memory. Run MacBERT, training-issue-only MacBERT-DAPT, GujiRoBERTa and SIKU with identical pinned W2NER heads and search budgets; compare W2NER, GlobalPointer and CRF on one frozen backbone. Select by the implemented issue-cluster bootstrap deltas or a documented speed/cost non-inferiority tradeoff, not an unsupported absolute 0.85 initial F1 gate. This separates OCR propagation from NER failure and prevents newspaper-template leakage.

The current production hypothesis is a cascade, subject to that benchmark: deterministic rules/gazetteers; the winner of the MacBERT/MacBERT-DAPT/GujiRoBERTa/SIKU W2NER tournament; GLiNER-X only if adding at least three raw-recoverable recall points costs no more than five precision points; and NuExtract3 only for disagreement, rare types, page-image context and implicit relations. Otter remains rights-blocked. Every stage can abstain. No model output creates an authority entity or reviewed claim without a separate link/review decision.
...
8. Have at least two historians author the eligible grounded-generation set, approve a pinned local or privacy-cleared remote model tournament, and run the implemented blind/adjudicated paired comparison before enabling narratives.
...
| NER | Rules + MacBERT/MacBERT-DAPT/GujiRoBERTa/SIKU W2NER tournament; gated GLiNER-X recall union; NuExtract3 difficult-case challenger | Multi-run benchmark harness and issue-cluster comparator implemented; Otter rights-blocked; historian gold/training/comparison pending |
...
| Evaluation | Frozen OCR/NER/RAG/generation judgments with evidence-validity and task metrics | OCR/NER contracts and scorers, retrieval smoke harness, and grounded-generation objective/blind/paired-human harness implemented; historian-authored sets and live model comparisons pending |
