...
2. **Supervised fixed-ontology tournament** for late-Qing/Republican-era Traditional Chinese—not ancient Chinese. First compare pinned W2NER, GlobalPointer and flat CRF heads on frozen MacBERT, then use the selected overlap-capable head with identical training budgets on target-relevant MacBERT, training-issue-only MacBERT-DAPT, mmBERT-base and tokenizer-qualified Chinese ModernBERT. Retain SIKU only as a target-period-evaluated control despite its ancient-book pretraining; defer license-gated GujiRoBERTa as an ancient-domain mismatch control. W2NER is the first implemented export path, not a winner: the cited EvaHan model-selection result favors it by only 1.15 F1 over GlobalPointer and explicitly warns of augmented-fold leakage. MacBERT is the first backbone because a directly relevant retyped *Shen Bao* study reports 58.26 F1 versus SIKU 56.83; neither score demonstrates OCR robustness. DAPT is the principal proposed challenger. mmBERT is a same-head ablation, not a new primary: its paper reports a WikiANN NER tie with XLM-R, a tokenizer weakness, and no target OCR result.
...
6. Run the frozen-backbone W2NER/GlobalPointer/CRF comparison, then the frozen selected-head MacBERT/MacBERT-DAPT/mmBERT/historical-encoder tournament, with GLiNER-X as the open-type recall challenger. Keep pinned GLiNER v2.5 low priority because it has no Chinese score and its whitespace-style splitting is unsafe for uninterrupted Chinese.
